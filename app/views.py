...
def extract():
    ...
    if response.status_code ==200:
        ...
        product_name=utils.extract_feautre(page_dom, "h1")
        opinions_count=utils.extract_feautre(page_dom, "a.product-review__link > span")
        if not opinions_count:
            error="Dla produktu o podanym id nie ma żadnej opinii"
    ...
    while next_page:
        ...
        if response.status_code ==200:
            ...
            try:
                next_page="https://www.ceneo.pl"+ utils.extract_feautre(page_dom, "a.pagination__next", "href")
            except TypeError:
                next_page= None
    ...
    stats= {
        ...
        "avarage_stars":float(opinions.stars.mean()),
        "pros":opinions.pros.explode().dropna().value_counts().to_dict(),
        "cons":opinions.cons.explode().dropna().value_counts().to_dict(),
        "recommendations": opinions.recommandation.value_counts(dropna=False).reindex(['Nie polecam', 'Polecam',  None],fill_value=0 ).to_dict()
...
def charts(product_id):
    ...
    plt.savefig(f"./app/static/images/charts/{stats['product_id']}, pie.png")
    plt.close()
